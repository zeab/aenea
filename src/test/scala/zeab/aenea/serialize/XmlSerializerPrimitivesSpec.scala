...
class XmlSerializerPrimitivesSpec extends MasterSuite {
  ...
  test("List[String]: Error Serialize") {
  ...
  test("Vector[String]: Error Serialize") {
  ...
  test("Map[String, String]: Error Serialize") {
  ...
  test("Seq[String]: Error Serialize") {
    val serializedXml: Either[Throwable, String] = Seq("1", "2", "3").asXml()
    val expectedError: String = "Must be a case class at root level cannot serialize : $colon$colon"
    compareErrorResults(expectedError, serializedXml)
  }

  test("Set[String]: Error Serialize") {
